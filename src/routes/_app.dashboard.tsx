...
import {
  kpis, liveThreats, riskDistribution, heatmap, attackCategories, fraudTrend,
  txStream, recentAlerts, recentInvestigations, blockedTx,
} from "@/lib/mock/data";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4 mb-6">
        <KpiCard label="Total Threats" value={kpis.totalThreats} delta={12} icon={<Shield className="h-4 w-4" />} />
        <KpiCard label="Critical" value={kpis.criticalThreats} delta={-8} icon={<AlertTriangle className="h-4 w-4" />} accent="var(--risk-critical)" gradient="from-rose-500/20 to-orange-500/20" />
        <KpiCard label="Fraud Prevented" value={kpis.fraudPreventedUsd} format={(n) => "$" + (n/1_000_000).toFixed(1) + "M"} delta={18} icon={<ShieldCheck className="h-4 w-4" />} accent="var(--risk-low)" gradient="from-emerald-500/20 to-teal-500/20" />
        <KpiCard label="Transactions Monitored" value={kpis.transactionsMonitored} delta={4} icon={<Activity className="h-4 w-4" />} />
        <KpiCard label="Avg. Risk Score" value={kpis.averageRiskScore} unit="/100" delta={-3} icon={<Gauge className="h-4 w-4" />} accent="var(--cyber-violet)" gradient="from-violet-500/20 to-fuchsia-500/20" />
        <KpiCard label="False Positive ↓" value={kpis.falsePositiveReduction} unit="%" delta={7} icon={<Sparkles className="h-4 w-4" />} accent="var(--cyber-cyan)" />
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <GlassCard className="col-span-12 xl:col-span-8 p-0">
          <div className="p-5 pb-3">
            <SectionHeader
              ...
              description="Auto-streaming from 12 correlated data planes"
              action={<span className="text-[10px] text-muted-foreground flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" /> streaming</span>}
            />
          </div>
          <div className="max-h-[380px] overflow-y-auto scrollbar-thin px-5 pb-5">
            <ol className="relative border-l border-white/8 ml-2 space-y-3">
              {liveThreats.slice(0, 14).map((t, i) => (
                <motion.li
                  key={t.id}
                ...
                >
                  <div className="absolute -left-[6px] mt-1.5"><SeverityDot severity={t.severity} pulse={t.severity === "critical"} /></div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-xs text-muted-foreground font-mono">{new Date(t.ts).toLocaleTimeString()}</span>
                    <RiskBadge severity={t.severity} />
                    <span className="text-sm font-medium">{t.type}</span>
                    <span className="text-[11px] text-muted-foreground">{t.source} · {t.entity}</span>
                    <span className="ml-auto text-xs font-mono text-muted-foreground">{t.ip} · {t.country}</span>
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <RiskBar value={t.risk} className="max-w-[220px]" />
                    <span className="text-[10px] text-muted-foreground font-mono">{t.risk}</span>
                  </div>
                </motion.li>
              ))}
            </ol>
        ...
        <GlassCard className="col-span-12 xl:col-span-8">
          ...
          <Heatmap
            data={heatmap}
            labelsX={Array.from({ length: 24 }, (_, i) => (i % 3 === 0 ? String(i).padStart(2,"0") : ""))}
        ...
        <GlassCard className="col-span-12 md:col-span-6 xl:col-span-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <div className="text-lg font-mono">2,847<span className="text-xs text-muted-foreground">/s</span></div>
              <Sparkline data={txStream.map((s) => s.approved)} color="var(--risk-low)" width={110} height={30} />
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground uppercase">Flagged</div>
              <div className="text-lg font-mono">91<span className="text-xs text-muted-foreground">/s</span></div>
              <Sparkline data={txStream.map((s) => s.flagged)} color="var(--risk-medium)" width={110} height={30} />
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground uppercase">Blocked</div>
              <div className="text-lg font-mono">14<span className="text-xs text-muted-foreground">/s</span></div>
              <Sparkline data={txStream.map((s) => s.blocked)} color="var(--risk-critical)" width={110} height={30} />
            </div>
          </div>
        ...
        <RecentPanel
          ...
          items={recentInvestigations.map((i) => ({
            id: i.id, primary: i.title, secondary: `${i.confidence}% confidence · $${(i.impactUsd/1000).toFixed(0)}K impact`, ts: i.ts, icon: <FileSearch2 className="h-3.5 w-3.5 text-violet-300" />,
          }))}
          href="/investigations"
        />
        <RecentPanel
          title="Recent Alerts"
          items={recentAlerts.slice(0,6).map((a) => ({
            id: a.id, primary: a.title, secondary: `${a.owner} · ${a.status}`, ts: a.ts, severity: a.severity,
          }))}
          href="/alerts"
        />
        <RecentPanel
          title="Recent Blocked Transactions"
          items={blockedTx.slice(0,6).map((t) => ({
            id: t.id, primary: `${t.currency} ${t.amount.toLocaleString()} · ${t.country}`, secondary: t.reason, ts: t.ts, icon: <Ban className="h-3.5 w-3.5 text-rose-300" />,
          }))}
          href="/transactions"
        />
      </div>
...
function RecentPanel({ title, items, href }: { title: string; items: { id: string; primary: string; secondary?: string; ts: number; icon?: React.ReactNode; severity?: import("@/lib/mock/data").Severity }[]; href: string }) {
  return (
    <GlassCard className="col-span-12 md:col-span-6 xl:col-span-4">
